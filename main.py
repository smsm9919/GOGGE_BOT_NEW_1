...
# إعدادات الجني الذكي
SCALP_TP_PCT = 0.0040  # 0.40% 
TREND_TP1_PCT = 0.0040  # 0.40% للمرحلة الأولى
TREND_TP2_PCT = 0.0100  # 1.00% للمرحلة الثانية
TREND_TP1_QTY_RATIO = 0.45  # 45% من المركز في TP1
FLOW_HOLD_THRESHOLD_A = 1.15
FLOW_HOLD_THRESHOLD_Z = 0.8
ADX_DROP_FOR_EXIT = 5
MAX_ABSORPTIONS_FOR_EXIT = 2
...
# Management profiles
TP1_PCT_SCALP = 0.0040
TP1_PCT_TREND = 0.0060
BE_AFTER_SCALP = 0.0030
BE_AFTER_TREND = 0.0040
TRAIL_ACT_SCALP = 0.0080
TRAIL_ACT_TREND = 0.0120
ATR_TRAIL_MULT = 1.6
...
TP1_PCT_BASE       = globals().get("TP1_PCT_BASE", 0.004)
...
# =================== EXECUTION HELPERS ===================
# إضافة الدوال المساعدة لإدارة التنفيذ الآمن
QTY_STEP = None
MIN_QTY = None

def _round_qty(q):
    """تقريب الكمية حسب خطوة التداول"""
    global QTY_STEP, MIN_QTY
    try:
        if QTY_STEP is None or QTY_STEP <= 0:
            return max(MIN_QTY or 0.001, float(q))
        stepped = math.floor(float(q) / QTY_STEP) * QTY_STEP
        return max(MIN_QTY or 0.001, stepped)
    except:
        return max(MIN_QTY or 0.001, float(q))

def _side_out(side_in):
    """تحويل اتجاه المركز إلى اتجاه الإغلاق"""
    return 'sell' if side_in in ('long','buy','BUY','LONG') else 'buy'

def _reduce_only_order(symbol, side_in, qty):
    """أمر إغلاق آمن بـ reduceOnly"""
    side = _side_out(side_in)
    q = _round_qty(float(qty))
    
    if MIN_QTY and q < MIN_QTY:
        log_w(f"⚠️ كمية الإغلاق صغيرة {q} < {MIN_QTY}")
        return False
        
    try:
        ex.create_order(symbol, 'market', side, q, params={'reduceOnly': True})
        log_g(f"✅ إغلاق آمن: {side} {q:.4f}")
        return True
    except Exception as e:
        log_w(f"❌ فشل الإغلاق الآمن: {e}")
        
        # محاولة ثانية بتخفيض 5% على الكمية
        try:
            q2 = _round_qty(q * 0.95)
            if not MIN_QTY or q2 >= MIN_QTY:
                ex.create_order(symbol, 'market', side, q2, params={'reduceOnly': True})
                log_g(f"✅ إغلاق معدل: {q2:.4f} بدلاً من {q:.4f}")
                return True
        except Exception as e2:
            log_e(f"⛔ فشل الإغلاق مرتين: {e2}")
            
        # محاولة أخيرة بدون reduceOnly (طارئة)
        try:
            ex.create_order(symbol, 'market', side, q)
            log_w(f"⚠️ إغلاق طارئ بدون reduceOnly: {q:.4f}")
            return True
        except Exception as e3:
            log_e(f"💥 فشل كامل في الإغلاق: {e3}")
            
    return False

def close_partial(pct, reason="partial"):
    """إغلاق جزئي للمركز"""
    if not STATE.get('open') or STATE.get('qty', 0) <= 0:
        log_w("⚠️ لا يوجد مركز للإغلاق الجزئي")
        return False
        
    qty = STATE['qty'] * float(pct)
    original_qty = STATE['qty']
    
    log_i(f"🔄 محاولة إغلاق جزئي: {pct*100:.1f}% = {qty:.4f} من {original_qty:.4f}")
    
    ok = _reduce_only_order(SYMBOL, STATE['side'], qty)
    if ok:
        new_qty = safe_qty(original_qty - qty)
        STATE['qty'] = new_qty
        log_g(f"✅ إغلاق جزئي ناجح: بقي {new_qty:.4f} من {original_qty:.4f} | السبب: {reason}")
        
        # إذا أصبحت الكمية صغيرة جداً، إغلاق كامل
        if new_qty <= FINAL_CHUNK_QTY:
            log_w(f"🔄 كمية متبقية صغيرة {new_qty:.4f}، إغلاق كامل...")
            return close_all("dust_after_partial")
            
        return True
    else:
        log_e(f"❌ فشل الإغلاق الجزئي: {pct*100:.1f}%")
        return False

def close_all(reason="manual"):
    """إغلاق كامل للمركز"""
    if not STATE.get('open') or STATE.get('qty', 0) <= 0:
        log_w("⚠️ لا يوجد مركز للإغلاق")
        return False
        
    qty = STATE['qty']
    log_i(f"🔄 محاولة إغلاق كامل: {qty:.4f} | السبب: {reason}")
    ...
    if ok:
        ...
        # تفعيل الانتظار للإشارة التالية
        global wait_for_next_signal_side
        prev_side = STATE.get('side')
        wait_for_next_signal_side = "sell" if prev_side == "long" else "buy"
        
        log_g(f"✅ إغلاق كامل ناجح: {qty:.4f} | انتظار {wait_for_next_signal_side}")
        return True
    else:
        log_e(f"❌ فشل الإغلاق الكامل: {qty:.4f}")
        return False
...
# =================== MODE DECISION ===================
def decide_trade_mode(df, ind, flow_data):
    """تحديد وضع التداول: SCALP أم TREND"""
    votes = 0
    
    # 1. قوة الترند (ADX + DI)
    adx = ind.get('adx', 0)
    di_spread = abs(ind.get('plus_di', 0) - ind.get('minus_di', 0))
    if adx >= 22 and di_spread >= 6:
        votes += 1
        log_i("MODE_VOTE: ADX+DI قوي")
    
    # 2. ترند RSI مستمر
    rsi_ctx = rsi_ma_context(df)
    if rsi_ctx["trendZ"] in ("bull", "bear") and rsi_ctx.get("persistence", 0) >= 3:
        votes += 1
        log_i(f"MODE_VOTE: RSI ترند مستمر ({rsi_ctx['trendZ']})")
    
    # 3. تدفق قوي
    if flow_data and flow_data.get("ok"):
        flow_z = abs(flow_data.get("delta_z", 0))
        if flow_z >= 0.6:
            votes += 1
            log_i(f"MODE_VOTE: تدفق قوي (z={flow_z:.2f})")
    
    # 4. محاذاة المتوسطات
    ma_trend = check_ma_alignment(df)
    if ma_trend in ("bull", "bear"):
        votes += 1
        log_i(f"MODE_VOTE: محاذاة MAs ({ma_trend})")
    
    # 5. الامتصاصات (خصم صوت)
    absorptions = count_absorptions(df)
    if absorptions >= 2:
        votes -= 1
        log_i(f"MODE_VOTE: امتصاصات عالية ({absorptions})")
    
    log_i(f"🎯 MODE DECISION: {votes}/4 votes → {'TREND' if votes >= 3 else 'SCALP'}")
    return "trend" if votes >= 3 else "scalp"

def check_ma_alignment(df, periods=[9, 21, 50]):
    """التحقق من محاذاة المتوسطات لصالح الترند"""
...
def count_absorptions(df, lookback=10):
    ...
    for i in range(1, lookback + 1):
        ...
        # شمعة امتصاص هابطة: قمة أعلى ولكن إغلاق أقل من منتصف الشمعة السابقة
        if current_high > prev_high and current_close < (prev_high + prev_low) / 2:
            absorptions += 1
        
        # شمعة امتصاص صاعدة: قاع أدنى ولكن إغلاق أعلى من منتصف الشمعة السابقة
        elif current_low < prev_low and current_close > (prev_high + prev_low) / 2:
            absorptions += 1
    
    return absorptions

# =================== ENHANCED TRADE MANAGEMENT ===================
def manage_scalp_position(pnl_pct, df, ind, info):
    """إدارة وضع SCALP - جني واحد"""
    if pnl_pct >= SCALP_TP_PCT * 100:  # تحويل للنسبة المئوية
        log_g(f"🎯 SCALP TP: +{pnl_pct:.2f}%")
        success = close_all("scalp_tp")
        
        if success:
            # كول داون لمدة 5-10 دقائق
            global wait_for_next_signal_side
            wait_for_next_signal_side = "sell" if STATE["side"] == "long" else "buy"
            log_i(f"⏳ SCALP COOLDOWN: انتظار {wait_for_next_signal_side} للإشارة التالية")

def manage_trend_position(pnl_pct, df, ind, info):
    """إدارة وضع TREND - جني على مرحلتين"""
    px = info["price"]
    side = STATE["side"]
    qty = STATE["qty"]
    
    # TP1: 45% من المركز عند 0.40%
    if not STATE.get("tp1_done") and pnl_pct >= TREND_TP1_PCT * 100:
        success = close_partial(TREND_TP1_QTY_RATIO, "trend_tp1")
        if success:
            STATE["tp1_done"] = True
            STATE["profit_targets_achieved"] += 1
            
            # تفعيل Breakeven + تشديد Trail بعد TP1
            STATE["breakeven_armed"] = True
            STATE["breakeven"] = STATE["entry"]
            if STATE.get("trail_active"):
                STATE["trail_tightened"] = True
            log_i("🛡️ BREAKEVEN ARMED + TRAIL TIGHTENED بعد TP1")
    
    # بعد TP1 - التحقق من شروط TP2 أو الخروج الصارم
    if STATE.get("tp1_done") and STATE.get("qty", 0) > 0:
        # Hold-TP: زيادة TP2 تدريجياً مع التدفق القوي
        current_tp2 = STATE.get("tp2_pct", TREND_TP2_PCT)
        flow_data = compute_flow_metrics(df)
        
        if flow_data and flow_data.get("ok"):
            flow_a = flow_data.get("imbalance", 1.0)
            flow_z = abs(flow_data.get("delta_z", 0))
            
            if ((side == "long" and flow_a >= FLOW_HOLD_THRESHOLD_A and flow_z >= FLOW_HOLD_THRESHOLD_Z) or
                (side == "short" and flow_a <= 1/FLOW_HOLD_THRESHOLD_A and flow_z >= FLOW_HOLD_THRESHOLD_Z)):
                new_tp2 = min(0.018, current_tp2 + 0.002)  # زيادة تدريجية حتى 1.8%
                if new_tp2 > current_tp2:
                    STATE["tp2_pct"] = new_tp2
                    log_i(f"📈 HOLD-TP: رفع TP2 إلى {new_tp2*100:.2f}%")
        ...
        # 1. إشارة RF عكسية
        rf_info = rf_signal_live(df)
        if (side == "long" and rf_info.get("short")) or (side == "short" and rf_info.get("long")):
            strict_exit = True
            exit_reason = "rf_opposite"
        
        # 2. امتصاصان متتاليان ضد الاتجاه
        absorptions = count_absorptions(df, lookback=5)
        if absorptions >= MAX_ABSORPTIONS_FOR_EXIT:
            strict_exit = True
            exit_reason = f"absorptions_{absorptions}"
        
        # 3. هبوط ADX + كروس RSI
        adx_current = ind.get('adx', 0)
        adx_prev = ind.get('adx_prev', adx_current)
        adx_drop = adx_prev - adx_current
        
        rsi_ctx = rsi_ma_context(df)
        rsi_cross_against = (side == "long" and rsi_ctx["cross"] == "bear") or (side == "short" and rsi_ctx["cross"] == "bull")
        
        if adx_drop >= ADX_DROP_FOR_EXIT and rsi_cross_against:
            strict_exit = True
            exit_reason = f"adx_drop_{adx_drop}+rsi_cross"
        
        # TP2 أو الخروج الصارم
        target_tp2 = STATE.get("tp2_pct", TREND_TP2_PCT)
        if pnl_pct >= target_tp2 * 100 or strict_exit:
            if strict_exit:
                log_w(f"🚨 TREND STRICT EXIT: {exit_reason} @ +{pnl_pct:.2f}%")
            else:
                log_g(f"🎯 TREND TP2: +{pnl_pct:.2f}%")
            
            close_all("trend_tp2" if not strict_exit else f"trend_strict_{exit_reason}")

def manage_after_entry_enhanced(df, ind, info):
    """إدارة محسنة للمركز مع نظام الجني الذكي حسب الوضع"""
    ...
    # تحديد الوضع إذا لم يكن محدداً
    if "mode" not in STATE:
        flow_data = compute_flow_metrics(df)
        STATE["mode"] = decide_trade_mode(df, ind, flow_data)
        log_g(f"🎯 الوضع المحدد: {STATE['mode'].upper()}")
    ...
    # جني الأرباح حسب الوضع
    if STATE["mode"] == "scalp":
        manage_scalp_position(pnl_pct, df, ind, info)
    else:  # TREND
        manage_trend_position(pnl_pct, df, ind, info)

    # إدارة Breakeven و Trailing Stop
...
def council_votes_pro_enhanced(df):
    """مجلس محسّن — بدون أي استدعاءات ل emit_snapshots لتفادي recursion"""
    ...
    try:
        ...
        addon_log_safe(f"council_votes_pro_enhanced error: {e}")
        return LAST_COUNCIL
    finally:
        COUNCIL_BUSY = False
...
def load_market_specs():
    global MARKET, AMT_PREC, LOT_STEP, LOT_MIN, QTY_STEP, MIN_QTY
    try:
        ex.load_markets()
        ...
        # تهيئة متغيرات الدقة العالمية
        QTY_STEP = LOT_STEP or 0.001
        MIN_QTY = LOT_MIN or 0.001
...
def emit_snapshots(exchange, symbol, df, balance_fn=None, pnl_fn=None, council=None):
    """
...
def open_market_enhanced(side, qty, price):
    ...
    # تحديد الوضع قبل الفتح
    flow_data = snap.get("flow", {})
    ind = compute_indicators(df)
    trade_mode = decide_trade_mode(df, ind, flow_data)
    
    votes = cv
    gz = snap["gz"]
    
    # تسجيل واضح مع عرض الوضع ونقاط الجني
    if trade_mode == "scalp":
        mgmt_info = {"tp_pct": SCALP_TP_PCT, "mode": "SCALP", "strategy": "جني واحد"}
    else:
        mgmt_info = {
            "tp1_pct": TREND_TP1_PCT, 
            "tp2_pct": TREND_TP2_PCT,
            "tp1_ratio": TREND_TP1_QTY_RATIO,
            "mode": "TREND", 
            "strategy": "جني على مرحلتين"
        }
    
    success = execute_trade_decision(side, price, qty, trade_mode, votes, gz)
    ...
    if success:
        STATE.update({
            ...
            "adds": 0,
            "mode": trade_mode,  # حفظ الوضع المحدد
            "management": mgmt_info
        })
        
        # تسجيل الفتح مع التفاصيل الكاملة
        log_trade_open(
        ...
        log_g(f"✅ POSITION OPENED: {side.upper()} | mode={trade_mode} | TP={'SCALP' if trade_mode == 'scalp' else 'TREND_2PHASE'}")
        return True
