# -*- coding: utf-8 -*-
"""
DOGE ULTRA PRO AI BOT - الإصدار الذكي المتقدم المتكامل
...
# =================== SMART GOLDEN ZONE VALIDATOR ===================

def _displacement_gz(closes):
    """قياس قوة الإزاحة للسعر"""
    if len(closes) < 22:
        return 0.0
    recent_std = closes.tail(20).std()
    current_move = abs(closes.iloc[-1] - closes.iloc[-5])  # حركة 5 فترات
    return current_move / max(recent_std, 1e-9)

def validate_golden_zone(side, current_price, df, ind, council_data, trend_ctx):
    """
    🧠 نظام تحقق متقدم من المناطق الذهبية
    - يتحقق من صحة المنطقة الذهبية قبل الدخول
    - يمنع الدخول في مناطق مزيفة أو ضعيفة
    - يتأكد من وجود جميع عناصر SMC اللازمة
    """
    
    # جلب بيانات المنطقة الذهبية
    gz_data = golden_zone_check(df, ind)
    
    if not gz_data.get("ok", False):
        return False, "not_golden_zone"
    
    zone_type = gz_data.get("zone", {}).get("type", "")
    
    # التحقق من تطابق الاتجاه مع نوع المنطقة
    if side == "buy" and zone_type != "golden_bottom":
        return False, f"zone_mismatch: {zone_type} for buy"
    if side == "sell" and zone_type != "golden_top":
        return False, f"zone_mismatch: {zone_type} for sell"
    
    # ---- 1) تحليل السيولة (Liquidity Analysis) ----
    liquidity_zones = smc_detector.detect_liquidity_zones(current_price)
    
    # للشراء: يجب وجود سيولة شراء تحت السعر
    if side == "buy":
        buy_liquidity = any(zone[0] == "buy_liquidity" for zone in liquidity_zones)
        if not buy_liquidity:
            return False, "no_buy_liquidity_below"
    
    # للبيع: يجب وجود سيولة بيع فوق السعر  
    if side == "sell":
        sell_liquidity = any(zone[0] == "sell_liquidity" for zone in liquidity_zones)
        if not sell_liquidity:
            return False, "no_sell_liquidity_above"
    
    # ---- 2) تحليل الإزاحة (Displacement) ----
    displacement = _displacement_gz(df['close'].astype(float))
    if displacement < 1.2:
        return False, f"weak_displacement: {displacement:.2f}"
    
    # ---- 3) تحليل Order Blocks ----
    ob_signal = detect_ob(df)
    if not ob_signal:
        return False, "no_valid_order_block"
    
    # التحقق من تطابق OB مع الاتجاه
    if side == "buy" and ob_signal[0] != "bullish":
        return False, f"ob_mismatch: {ob_signal[0]} for buy"
    if side == "sell" and ob_signal[0] != "bearish":
        return False, f"ob_mismatch: {ob_signal[0]} for sell"
    
    # ---- 4) تحليل الحجم (Volume Analysis) ----
    volume_profile = compute_volume_profile(df)
    if not volume_profile.get('volume_spike', False):
        return False, "no_volume_spike"
    
    # ---- 5) بوابات RSI/ADX ----
    rsi_val = safe_get(ind, 'rsi', 50)
    adx_val = safe_get(ind, 'adx', 0)
    
    if side == "buy":
        if rsi_val < 32:
            return False, f"rsi_oversold_risk: {rsi_val:.1f}"
        if rsi_val > 65:
            return False, f"rsi_overbought_for_buy: {rsi_val:.1f}"
    else:  # sell
        if rsi_val > 68:
            return False, f"rsi_overbought_risk: {rsi_val:.1f}"
        if rsi_val < 35:
            return False, f"rsi_oversold_for_sell: {rsi_val:.1f}"
    
    if adx_val < 18:
        return False, f"weak_trend_adx: {adx_val:.1f}"
    
    # ---- 6) تحقق من قوة مجلس الإدارة ----
    council_score = council_data.get('score_b', 0) if side == "buy" else council_data.get('score_s', 0)
    council_votes = council_data.get('b', 0) if side == "buy" else council_data.get('s', 0)
    
    if council_score < 15.0:
        return False, f"weak_council_score: {council_score:.1f}"
    if council_votes < 8:
        return False, f"low_council_votes: {council_votes}"
    
    # ---- 7) تحقق من قوة الترند ----
    if not trend_ctx.is_strong_trend():
        return False, "weak_trend_context"
    
    # ---- 8) تحقق من Fair Value Gap ----
    fvg_signal = detect_fvg(df)
    if fvg_signal:
        fvg_type = fvg_signal[0]
        if side == "buy" and fvg_type != "bullish":
            return False, f"fvg_mismatch: {fvg_type} for buy"
        if side == "sell" and fvg_type != "bearish":
            return False, f"fvg_mismatch: {fvg_type} for sell"
    
    # ✅ كل الشروط متحققة - المنطقة الذهبية صالحة
    return True, "golden_zone_validated"

def is_scalp_allowed(rsi_val, adx_val, trend_strength):
    """
    🚫 منع السكالب في مناطق التشبع والترند الضعيف
    """
    # منع السكالب في RSI متطرف
    if rsi_val < 30 or rsi_val > 70:
        return False, f"rsi_extreme: {rsi_val:.1f}"
    
    # منع السكالب في ترند ضعيف
    if adx_val < 15:
        return False, f"weak_trend_adx: {adx_val:.1f}"
    
    # السماح بالسكالب فقط في ظروف محددة
    if trend_strength in ["weak", "flat"] and 40 <= rsi_val <= 60:
        return True, "optimal_scalp_conditions"
    
    return True, "scalp_allowed"

def log_golden_entry(side, price, qty, validation_data):
    """
    🎨 لوج ملوّن ومفصل لدخول المناطق الذهبية
    """
    zone_type = "GOLDEN_BOTTOM" if side == "buy" else "GOLDEN_TOP"
    ...
    │ 
    │ 📊 VALIDATION METRICS:
    │ ├── 🧠 Council Score: {validation_data['council_score']:.1f}
    │ ├── 📈 ADX: {validation_data['adx']:.1f} | RSI: {validation_data['rsi']:.1f}
    │ ├── 💧 Liquidity: {validation_data['liquidity']}
    │ ├── 📦 Order Block: {validation_data['ob_type']}
    │ ├── 🔥 Displacement: {validation_data['displacement']:.2f}
    │ └── 📊 Volume: {validation_data['volume_status']}
    │ 
    │ ✅ VALIDATION: {validation_data['validation_reason']}
    └─────────────────────────────────────────────────────
    ...
    logging.info(f"GOLDEN_ENTRY | {side}@{price} | "
                f"score={validation_data['council_score']:.1f} | "
                f"adx={validation_data['adx']:.1f} | "
                f"rsi={validation_data['rsi']:.1f}")

def setup_golden_trade_management(side, mode):
    """
    ⚡ إعدادات متقدمة لإدارة صفقات المناطق الذهبية
    """
    if mode == "golden_trend":
        return {
            "tp1_pct": 1.2,    # TP1 عند 1.2%
            "tp2_pct": 2.5,    # TP2 عند 2.5%  
            "tp3_pct": 4.0,    # TP3 عند 4.0%
            "tp1_fraction": 0.25,  # إغلاق 25% عند TP1
            "tp2_fraction": 0.35,  # إغلاق 35% عند TP2
            "tp3_fraction": 0.40,  # إغلاق 40% عند TP3
            "be_activate_pct": 0.8,  # تفعيل التعادل عند 0.8%
            "trail_activate_pct": 1.5,  # تفعيل التريل عند 1.5%
            "atr_trail_mult": 1.5,  # مضاعف التريل
            "close_aggression": "low",  # صبر أكبر في الخروج
            "profile": "GOLDEN_TREND",
            "profile_desc": "صفقة ترند في منطقة ذهبية - صبر واحترافية"
        }
    else:
        return setup_trade_management(mode)

def monitor_golden_zone_quality(df, ind, current_price, side):
    """
    📊 مراقبة مستمرة لجودة المنطقة الذهبية أثناء الصفقة
    """
    if not STATE.get("is_golden_trade", False):
        return "not_golden_trade"
    
    gz_data = golden_zone_check(df, ind)
    if not gz_data.get("ok", False):
        return "golden_zone_lost"
    
    # التحقق من استمرارية قوة المنطقة
    zone_score = gz_data.get("score", 0)
    if zone_score < 4.0:
        return f"zone_weakened: {zone_score:.1f}"
    
    # التحقق من عدم اختراق المنطقة
    zone = gz_data.get("zone", {})
    if side == "long" and current_price < zone.get("f786", 0):
        return "zone_broken_below"
    if side == "short" and current_price > zone.get("f618", 0):
        return "zone_broken_above"
...
LEVERAGE   = int(os.getenv("LEVERAGE", 10))          # رافعة ثابتة 10x
RISK_ALLOC = float(os.getenv("RISK_ALLOC", 0.60))    # 60% من رصيد المحفظة في كل صفقة
...
# RF Settings - Optimized for DOGE
RF_SOURCE = "close"
...
# Dynamic TP / trail - Optimized for DOGE
TP1_PCT_BASE       = 0.45
TP1_CLOSE_FRAC     = 0.50
...
def detect_super_scalp_opportunity(df, ind, flow, volume_profile, momentum, spread_bps):
    try:
        ...
        # 🆕 تحقق من صلاحية السكالب
        rsi_val = safe_get(ind, 'rsi', 50)
        adx_val = safe_get(ind, 'adx', 0)
        trend_strength = compute_trend_strength(df, ind).get("strength", "flat")
        scalp_allowed, scalp_reason = is_scalp_allowed(rsi_val, adx_val, trend_strength)
        if not scalp_allowed:
            return (None, f"scalp_blocked: {scalp_reason}")
...
def open_market_enhanced(side, qty, price):
    ...
    # 🆕 التحقق مما إذا كانت صفقة منطقة ذهبية
    is_golden_trade = False
    if STATE.get("last_entry_reasons", "") and "GOLDEN_VALIDATED" in STATE.get("last_entry_reasons", ""):
    ...
    # 🆕 إعدادات الإدارة بناءً على نوع الصفقة (عادية أو ذهبية)
    if is_golden_trade:
        management_config = setup_golden_trade_management(side, "golden_trend")
        STATE["is_golden_trade"] = True
    else:
        management_config = {
    ...
    if success:
        ...
        if is_golden_trade:
            validation_data = {
                ...
                'rsi': safe_get(ind, 'rsi', 50),
                'liquidity': "confirmed",
                'ob_type': detect_ob(df)[0] if detect_ob(df) else "none",
                'displacement': _displacement_gz(df['close'].astype(float)),
                'volume_status': "spike_detected",
                'validation_reason': "GOLDEN_ZONE_VALIDATED"
            }
            log_golden_entry(side, price, qty, validation_data)
        else:
             else:
    # نوع الملف واضح + تصنيف البروفايل
...
STATE = {
    ...
    "profit_targets_achieved": 0,
    "is_golden_trade": False  # 🆕 إضافة حالة المناطق الذهبية
}
compound_pnl = 0.0
...
def _reset_after_close(reason, prev_side=None):
    ...
    STATE.update({
        ...
        "is_golden_trade": False  # 🆕 إعادة تعيين حالة المناطق الذهبية
    })
...
def manage_trade_by_profile(df, ind, info):
    ...
    # 🆕 مراقبة جودة المنطقة الذهبية إذا كانت الصفقة ذهبية
    if STATE.get("is_golden_trade", False):
        zone_health = monitor_golden_zone_quality(df, ind, px, side)
        if zone_health != "zone_healthy":
            log_w(f"⚠️ GOLDEN ZONE HEALTH: {zone_health}")
            # إذا كانت المنطقة ضعيفة جدًا، نغلق جزءًا من الصفقة
            if "zone_weakened" in zone_health or "zone_broken" in zone_health:
                close_qty = safe_qty(STATE["qty"] * 0.5)  # إغلاق 50%
                if close_qty > 0:
                    ...
                    if MODE_LIVE and EXECUTE_ORDERS and not DRY_RUN:
                        try:
                            params = exchange_specific_params(close_side, is_close=True)
                            ex.create_order(SYMBOL, "market", close_side, close_qty, None, params)
                            log_g(f"🛡️ GOLDEN ZONE PROTECTION | closed 50% due to {zone_health}")
                            STATE["qty"] = safe_qty(STATE["qty"] - close_qty)
                        except Exception as e:
                            log_e(f"❌ Golden zone protection close failed: {e}")
    ...
    while True:
        try:
            ...
            if final_signal is None and council_side is not None:
                ...
                # 🆕 إضافة التحقق من المناطق الذهبية
                if council_side in ["buy", "sell"]:
                    is_valid_golden, validation_reason = validate_golden_zone(
                        council_side, 
                        px or info["price"], 
                        df, 
                        ind, 
                    ...
                    if not is_valid_golden:
                        safe_to_enter = False
                        log_i(f"🏛 COUNCIL ENTRY BLOCKED: {validation_reason}")
                    else:
                        entry_reasons.append(f"GOLDEN_VALIDATED: {validation_reason}")
                        log_g(f"🏆 GOLDEN ZONE VALIDATED for {council_side.upper()} | {validation_reason}")
                ...
                if safe_to_enter:
                    ...
                    log_g(f"🏛 COUNCIL STRONG ENTRY → {final_signal.upper()} | votes={cb}/{cs} score={sb:.1f}/{ss:.1f} conf={conf:.2f}")
                else:
...
def pretty_snapshot(bal, info, ind, spread_bps, reason=None, df=None):
    if LOG_LEGACY:
        ...
        if STATE["open"]:
            ...
            print(f"   🏆 GOLDEN_TRADE={STATE.get('is_golden_trade', False)}")
        else:
            print("   ⚪ FLAT")
