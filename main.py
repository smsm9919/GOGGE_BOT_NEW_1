...
MID_TREND_MIN_SCORE = 4.0         # الحد الأدنى لدرجة Council للدخول
MID_TREND_HTF_REQ = True          # هل يتطلب تأكيد من HTF؟
MID_TREND_FLOW_REQ = False        # هل يتطلب تأكيد من Flow؟
MID_TREND_WICK_REQ = True         # هل يتطلب وجود wick؟
MID_TREND_MAX_HOLD_BARS = 4       # أقصى عدد شمعات للاحتفاظ بالصفقة
MID_TREND_TP_PCT = 0.0050         # 0.50% هدف ربح
MID_TREND_BE_PCT = 0.0030         # 0.30% تفعيل Breakeven
MID_TREND_TRAIL_PCT = 0.0060      # 0.60% تفعيل التريل
...
def log_trade_open(*, side:str, price:float, qty:float, leverage:int,
    ...
    # إضافة أيقونة النمط
    mode_icon = "⚡" if mode == "scalp" else "🌊" if mode == "mid_trend" else "🚀" if mode == "strong_trend" else "ℹ️"
    ...
    if any(v is not None for v in (tp1, bea, tra)):
        mg_part = " | MGMT:" \
                  ...
                  + (f" MaxBars={mg.get('max_hold_bars', 10)}")
...
# =================== MID TREND DETECTION ===================
def detect_mid_trend_opportunity(df, council_data, htf_ctx, candles_analysis):
    """كشف فرص الدخول في الترند المتوسط"""
    ind = council_data["ind"]
    adx = ind.get('adx', 0)
    
    # التحقق من شروط MID_TREND الأساسية
    if not (MID_TREND_ADX_MIN <= adx < MID_TREND_ADX_MAX):
        return None
    
    # التحقق من تأكيد HTF إذا مطلوب
    if MID_TREND_HTF_REQ and htf_ctx.get("trend") == "none":
        return None
    
    # تحليل الشمعة الحالية
    wick_signal = False
    if MID_TREND_WICK_REQ and len(df) >= 2:
        current = df.iloc[-1]
        prev = df.iloc[-2]
        ...
        if total_range > 0:
            ...
            # Wick كبير بالنسبة للبودي
            if (upper_wick > body_size * 1.5) or (lower_wick > body_size * 1.5):
                wick_signal = True
                
                # تحديد اتجاه بناءً على نوع Wick
                if lower_wick > body_size * 1.5 and current_close > current_open:
                    # Hammer-like pattern في ترند صاعد
                    return {"action": "mid_trend_buy", "reason": "wick_rejection_bull", "wick_type": "lower"}
                elif upper_wick > body_size * 1.5 and current_close < current_open:
                    # Shooting star في ترند هابط
                    return {"action": "mid_trend_sell", "reason": "wick_rejection_bear", "wick_type": "upper"}
    
    # إذا لم يكن هناك wick، نتحقق من شروط أخرى
    score_b = council_data["score_b"]
    score_s = council_data["score_s"]
    
    mid_trend_buy = (
        score_b >= MID_TREND_MIN_SCORE and
        council_data["b"] > council_data["s"] and
        ind.get('rsi', 50) < 65 and
        htf_ctx.get("trend") in ["bull", "none"]
    )
    
    mid_trend_sell = (
        score_s >= MID_TREND_MIN_SCORE and
        council_data["s"] > council_data["b"] and
        ind.get('rsi', 50) > 35 and
        htf_ctx.get("trend") in ["bear", "none"]
    )
    
    if mid_trend_buy:
        return {"action": "mid_trend_buy", "reason": f"council_score_{score_b:.1f}", "wick_signal": wick_signal}
    elif mid_trend_sell:
        return {"action": "mid_trend_sell", "reason": f"council_score_{score_s:.1f}", "wick_signal": wick_signal}
...
def decide_strategy_mode(df, adx=None, di_plus=None, di_minus=None, rsi_ctx=None, htf_ctx=None):
    """تحديد نمط التداول: STRONG_TREND / MID_TREND / SCALP"""
    ...
    # STRONG_TREND - شروط صارمة
    strong_trend = (
        (adx >= ADX_TREND_MIN and di_spread >= DI_SPREAD_TREND) or
        (rsi_ctx["trendZ"] in ("bull", "bear") and not rsi_ctx["in_chop"] and adx >= 25)
    )
    
    # MID_TREND - شروط متوسطة
    mid_trend = (
        (MID_TREND_ADX_MIN <= adx < MID_TREND_ADX_MAX) and
        (di_spread >= 3.0) and  # انتشار معقول بين DI
        (not rsi_ctx["in_chop"]) and  # خارج النطاق الحيادي
        (htf_ctx.get("trend") != "none")  # HTF يؤيد الاتجاه
    )
    
    if strong_trend:
        return {"mode": "strong_trend", "why": "strong_trend_adx/di"}
    elif mid_trend:
        return {"mode": "mid_trend", "why": f"mid_trend_adx_{adx:.1f}_htf_{htf_ctx.get('trend')}"}
    else:
        return {"mode": "scalp", "why": "scalp_default"}
...
def setup_trade_management(mode):
    ...
    if mode == "strong_trend":
        ...
        return {
            "tp1_pct": MID_TREND_TP_PCT,
            "be_activate_pct": MID_TREND_BE_PCT,
            "trail_activate_pct": MID_TREND_TRAIL_PCT,
            "atr_trail_mult": ATR_TRAIL_MULT,
            "close_aggression": "high",  # خروج أسرع
            "max_hold_bars": MID_TREND_MAX_HOLD_BARS,
            "partial_close_pct": 0.35  # نسبة إغلاق جزئي أكبر
        }
    else:  # scalp
        return {
            ...
            "atr_trail_mult": ATR_TRAIL_MULT,
            "close_aggression": "very_high",
            "max_hold_bars": FAST_MAX_HOLD_BARS,
            "partial_close_pct": 0.40
        }
...
def emit_snapshots(exchange, symbol, df, balance_fn=None, pnl_fn=None):
    ...
    try:
        ...
        strat_icon = "⚡" if mode["mode"]=="scalp" else "🌊" if mode["mode"]=="mid_trend" else "🚀" if mode["mode"]=="strong_trend" else "ℹ️"
        strat = f"Strategy: {strat_icon} {mode['mode'].upper()} ({mode.get('why','')})"
        ...
        if LOG_ADDONS:
            ...
            # إضافة معلومات MID_TREND في اللوج
...
def manage_after_entry_enhanced(df, ind, info):
    ...
    # فحص الحد الأقصى للشمعات حسب النمط
    max_hold_bars = management.get("max_hold_bars", 10)
    if STATE["bars_in_trade"] >= max_hold_bars:
        log_w(f"⏰ MAX HOLD BARS REACHED ({STATE['bars_in_trade']}/{max_hold_bars}) - Closing position")
        close_market_strict("max_hold_time")
        return
    ...
    if LOG_LEGACY:
        ...
        if STATE["open"]:
            ...
            if STATE.get('tbe_zone'):
                zone = STATE['tbe_zone']
                print(f"   🚀 TBE Zone: {zone['type']} [{fmt(zone['low'])}-{fmt(zone['high'])}]")
        else:
            print("   ⚪ FLAT")
...
def metrics():
    return jsonify({
        ...
        "mid_trend_system": {
            "enabled": True,
            "adx_range": [MID_TREND_ADX_MIN, MID_TREND_ADX_MAX],
            "min_score": MID_TREND_MIN_SCORE
        }
    })
